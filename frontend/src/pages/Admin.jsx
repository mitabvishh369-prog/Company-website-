  ...
  return (
    <div data-testid="admin-page"><Toaster richColors position="top-right"/>
      ...
      <section className="max-w-[1440px] mx-auto px-6 lg:px-10 grid grid-cols-2 md:grid-cols-4 gap-3 mt-10">
        {[['Users', stats.users],['Events (approved)', stats.events_approved],['Classes (approved)', stats.classes_approved],['Artists (approved)', stats.artists_approved],['Events pending', stats.events_pending],['Classes pending', stats.classes_pending],['Artists pending', stats.artists_pending],['Bookings', stats.bookings]].map(([k,v])=>(
      ...
      <section className="max-w-[1440px] mx-auto px-6 lg:px-10 mt-10 border-b border-[color:var(--line-dark)]">
        <div className="flex gap-6 overflow-x-auto">
          {["events","classes","artists","bookings"].map(t=><button key={t} onClick={()=>setTab(t)} data-testid={`admin-tab-${t}`} className={`py-4 label-eyebrow ${tab===t?'text-[color:var(--accent)] border-b-2 border-[color:var(--accent)] -mb-px':'text-[color:var(--ink-2)]'}`}>{t}</button>)}
        </div>
      </section>
        ...
        {tab!=='bookings' ? (
          ...
          <div className="space-y-3">{pending[tab].map(it=>(
            <div key={it.id} className="card p-5 flex items-start gap-4" data-testid={`admin-${tab}-row`}>
              <div className="w-24 h-24 bg-[color:var(--bg-3)] shrink-0"><img src={fileUrl(it.poster_url||it.avatar_url||it.images?.[0]||it.portfolio_images?.[0])} className="w-full h-full object-cover"/></div>
