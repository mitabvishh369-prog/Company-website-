...
import { Calendar, MapPin, Ticket, Users, Trophy, ArrowUpRight, Instagram, Youtube, Globe, X } from "lucide-react";
...
export function EventDetail() {
  ...
  const pay = async () => {
    ...
    setBusy(true);
    try {
      const { data } = await api.post("/payments/checkout", { kind: "event", ref_id: id, origin_url: window.location.origin });
      if (data.free) { toast.success("Registered — see you there!"); nav("/dashboard"); }
  ...
  return (
    <div data-testid="event-detail"><Toaster/>
      <section className="max-w-[1440px] mx-auto px-6 lg:px-10 pt-10 grid lg:grid-cols-12 gap-10">
        <div className="lg:col-span-7">
          ...
          <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-6 hairline pb-8">
            <Meta i={<Calendar size={16}/>} k="Date" v={d.date?.slice(0,10)}/>
            <Meta i={<MapPin size={16}/>} k="Venue" v={`${d.venue}, ${d.city}`}/>
            <Meta i={<Users size={16}/>} k="Capacity" v={d.capacity || '—'}/>
            <Meta i={<Trophy size={16}/>} k="Prize" v={d.prize_money?`$${d.prize_money.toLocaleString()}`:'—'}/>
          </div>
          <div className="mt-8">
          ...
          {d.sponsors?.length > 0 && <Block title="Sponsors" items={d.sponsors}/>}
        ...
        <aside className="lg:col-span-5">
          <div className="sticky top-24">
            <div className="aspect-[4/5] bg-[color:var(--bg-3)] img-hover">
              <img src={fileUrl(d.poster_url || d.images?.[0]) || "https://images.unsplash.com/photo-1604954055722-7f80571fbfc3?crop=entropy&cs=srgb&fm=jpg&q=85"} alt={d.title} className="w-full h-full object-cover"/>
            </div>
            <div className="mt-6 p-6 border border-[color:var(--ink)] bg-white">
              ...
              <div className="mt-2 font-display text-4xl font-bold">{d.registration_fee ? `$${d.registration_fee}` : 'FREE'}</div>
              <div className="text-xs text-[color:var(--ink-2)] mt-1">Deadline · {d.registration_deadline?.slice(0,10) || 'Open'}</div>
              <button onClick={pay} disabled={busy} data-testid="event-book-btn" className="btn-accent w-full justify-center mt-6">{busy?'…':d.registration_fee>0?'Register & pay →':'Register (Free) →'}</button>
...
export function ClassDetail() {
  ...
  const pay = async () => {
    ...
    setBusy(true);
    try {
      const { data } = await api.post("/payments/checkout", { kind: "class", ref_id: id, origin_url: window.location.origin });
        ...
        <aside className="lg:col-span-5">
          <div className="sticky top-24">
            ...
            <div className="mt-6 p-6 border border-[color:var(--ink)] bg-white">
              ...
              <div className="font-display text-4xl font-bold mt-2">{d.fee ? `$${d.fee}` : 'FREE'}</div>
...
function BookingModal({ artist, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={onClose} data-testid="booking-modal">
        ...
        <form onSubmit={submit} className="p-8 space-y-4">
          {[["company_name","Company / Brand"],["contact_person","Contact person"],["email","Email","email"],["phone","Phone"],["project_name","Project / Event name"],["city","City"],["date","Date","date"],["budget","Budget (optional)","number"]].map(([k,l,t])=>(
            <label key={k} className="block">
              <span className="label-eyebrow block mb-2">{l}</span>
              <input type={t||'text'} value={f[k]} required={k!=='budget'} data-testid={`booking-${k}`}
                     onChange={e=>setF({...f, [k]: e.target.value})}
                     className="w-full px-4 py-3 border border-[color:var(--line)] focus:border-[color:var(--accent)] focus:outline-none"/>
            </label>
          ))}
          <label className="block">
            <span className="label-eyebrow block mb-2">Requirements</span>
            <textarea rows={4} value={f.requirements} required data-testid="booking-requirements"
          ...
          <p className="text-xs text-[color:var(--muted)] text-center">Our team reviews every request within 24h to match availability, pricing & fit.</p>
        </form>
      </div>
    </div>
  );
